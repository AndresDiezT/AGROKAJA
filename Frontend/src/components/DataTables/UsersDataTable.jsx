    ...
    return (
        <div className="custom-table rounded-lg shadow">
            ...
            <div className="w-full overflow-x-auto">
                <table className="w-full text-sm hidden table-fixed xl:table">
                    <thead className="sticky top-0">
                        {table.getHeaderGroups().map((headerGroup) => (
                            <tr key={headerGroup.id}>
                                {headerGroup.headers.map((header) => (
                                    <th key={header.id} className="p-2 text-left break-words" style={{ minWidth: "150px" }}>
                        ...
                        {table.getRowModel().rows.map((row) => (
                            <tr key={row.id} className="border-b border-base-300 hover:bg-base-300">
                                {row.getVisibleCells().map((cell) => (
                                    <td key={cell.id} className="p-2 break-words" style={{ minWidth: "150px" }}>
                                        {flexRender(
