...
import { useRolesAdmin } from "../../../hooks/queries/useRole"
import {
    useActivateRole,
    useDeactivateRole,
} from "../../../hooks/mutations/useRolesMutation"
...
function cleanFilters(filters) {
    ...
    if (!cleaned.nameRole?.trim()) delete cleaned.nameRole
    if (!cleaned.guardNameRole?.trim()) delete cleaned.guardNameRole
...
export default function CityList() {
    ...
    const { data: roles = [], isLoading, isError, error, refetch } = useRolesAdmin(cleanedFilters)
    ...
    const handleToggleActive = (role) => {
        if (role.isActive) {
            deactivateRole(role.idRole, {
                onError: (err) => {
                    console.error("Error al desactivar:", err)
                },
            })
        } else {
            activateRole(role.idRole, {
                onError: (err) => {
        ...
        <>
            <div className="mb-4 flex flex-wrap gap-3 items-center">
                ...
                <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                    <button
                        className={`px-3 py-1 rounded-full cursor-pointer text-white ${filters.isActive === undefined ? 'bg-blue-500 text-white' : 'bg-gray-700'}`}
                        onClick={() => setFilters({ ...filters, isActive: undefined })}
                    >
                        Todos
                    </button>
                    <button
                        className={`px-3 py-1 rounded-full cursor-pointer text-white ${filters.isActive === true ? 'bg-green-500 text-white' : 'bg-gray-700'}`}
                        onClick={() => setFilters({ ...filters, isActive: true })}
                    >
                        Activos
                    </button>
                    <button
                        className={`px-3 py-1 rounded-full cursor-pointer text-white ${filters.isActive === false ? 'bg-red-500 text-white' : 'bg-gray-700'}`}
            ...
            <ConfigDataTable
                ...
                editUrl={(rowData) => `/staff/settings/roles/edit/${rowData.idRole}`}
                detailUrl={(rowData) => `/staff/settings/roles/${rowData.idRole}`}
